...
class DataKey:
    ...
    env: Optional[str]  # None when visible to every environment
    module: Optional[str]
    digest: str
...
def build_inventory(config: RunConfig,
                    environments: List[Environment]) -> Inventory:
    inv = Inventory()
    ...
    if config.global_hiera:
        if config.global_hiera.is_file():
            ...
            if hiera.usable:
                _scan_layer(inv, config, hiera,
                            base_dir=config.global_hiera.parent,
                            layer=LAYER_GLOBAL, env=None, module=None)
        else:
            inv.warnings.append(Warn(
                "hiera_config",
                "global hiera.yaml not found at %s (continuing without "
                "a global layer)" % config.global_hiera))
    ...
    for extra in config.extra_datadirs:
        if not extra.is_dir():
            inv.warnings.append(Warn(
                "hiera_config", "--extra-datadir %s does not exist" % extra))
            continue
        entry = HierarchyEntry(
            name="(extra-datadir)", datadir_raw=str(extra),
            backend_name="yaml_data", backend_kind="yaml",
            patterns=["**"], glob_flags=[True])
        _register_datadir(inv, extra, LAYER_SHARED, env=None, module=None,
                          entry=entry)
    return inv


def scan_module_data(inv: Inventory, config: RunConfig, module_dir: Path,
                     module_name: str, env: Optional[str]) -> None:
    ...
    inv.warnings.extend(hiera.warnings)
    if hiera.usable:
        _scan_layer(inv, config, hiera, base_dir=module_dir,
                    layer=LAYER_MODULE, env=env, module=module_name)


def _scan_environment(inv: Inventory, config: RunConfig,
    ...
    if hiera_path.is_file():
        ...
        inv.warnings.append(Warn(
            "hiera_config",
            "environment '%s' has no hiera.yaml; assuming default "
            "hierarchy (data/common.yaml)" % env.name))
    if not hiera.usable:
        hiera.entries = [HierarchyEntry(
            name="(default hierarchy)", datadir_raw="data",
            backend_name="yaml_data", backend_kind="yaml",
            patterns=["common.yaml"], glob_flags=[False],
            config_file=hiera_path)]
    inv.env_hiera[env.name] = hiera
    _scan_layer(inv, config, hiera, base_dir=env.path,
                layer=LAYER_ENVIRONMENT, env=env.name, module=None)


def _scan_layer(inv: Inventory, config: RunConfig, hiera: HieraConfig,
                base_dir: Path, layer: str, env: Optional[str],
        ...
        for datadir in _resolve_datadirs(inv, config, entry, base_dir):
            ...
            if layer == LAYER_ENVIRONMENT and env is not None:
                try:
                    ...
                    entry_env = None
            _register_datadir(inv, datadir, entry_layer, entry_env, module,
                              entry)
...
def _resolve_datadirs(inv: Inventory, config: RunConfig,
                      entry: HierarchyEntry, base_dir: Path) -> List[Path]:
    raw = entry.datadir_raw
    context = "hierarchy entry '%s' in %s" % (entry.name, entry.config_file)
    ...
    if "%{" in raw:
        ...
        if not matches:
            inv.warnings.append(Warn(
                "hiera_config",
                "%s: interpolated datadir %r matches no directories "
                "(tried %s)" % (context, raw, pattern)))
    ...
    if os.path.isabs(raw):
        ...
        if raw.startswith(DEFAULT_CODE_PREFIX):
            ...
            if candidate.is_dir():
                inv.warnings.append(Warn(
                    "hiera_config",
                    "%s: datadir %s not found; rebased under code dir as %s"
                    % (context, raw, candidate)))
                return [candidate.resolve()]
        candidate = config.code_dir / path.name
        if candidate.is_dir():
            inv.warnings.append(Warn(
                "hiera_config",
                "%s: datadir %s not found; using %s from the code dir "
                "instead" % (context, raw, candidate)))
            return [candidate.resolve()]
        inv.warnings.append(Warn(
            "hiera_config", "%s: datadir %s does not exist (use "
            "--extra-datadir to point hiera-gc at a copy)" % (context, raw)))
        return []
    ...
    if path.is_dir():
        return [path.resolve()]
    inv.warnings.append(Warn(
        "hiera_config", "%s: datadir %s does not exist" % (context, path)))
    return []


def _register_datadir(inv: Inventory, datadir: Path, layer: str,
                      env: Optional[str], module: Optional[str],
                      entry: HierarchyEntry) -> None:
    scan = inv.scans.get(datadir)
    if scan is None:
        scan = DataDirScan(datadir=datadir, layer=layer, env=env,
                           module=module)
        inv.scans[datadir] = scan
        _scan_files(inv, scan)
    if not any(e.config_file == entry.config_file and e.index == entry.index
...
def _scan_files(inv: Inventory, scan: DataDirScan) -> None:
    ...
    for path in sorted(scan.datadir.rglob("*")):
        ...
        if any(part.startswith(".") for part in
               path.relative_to(scan.datadir).parts):
            continue
        real = path.resolve()
        if real in seen:
            continue
...
def _load_keys(inv: Inventory, scan: DataDirScan, path: Path,
               real: Path) -> None:
    ...
    if doc is None:
        try:
            ...
            inv.warnings.append(Warn(
                "parse_error", "cannot parse data file: %s" % exc.message,
                file=str(path), line=exc.line))
            return
        inv.docs[real] = doc
        for problem in doc.problems:
            inv.warnings.append(Warn("data_file", problem, file=str(path)))
    for top in doc.keys:
        inv.keys.append(DataKey(
            name=top.name, file=path, line=top.line, layer=scan.layer,
            env=scan.env, module=scan.module, digest=top.digest(),
