...
def extract_ruby(text: str, file: Path) -> List[Consumer]:
    ...
    for match in CALL_FUNCTION.finditer(text):
        consumers.append(Consumer(
            kind="ruby_lookup", key=match.group(1), pattern=None, file=file,
            line=_line(text, match.start()), detail="call_function lookup"))
    for match in LOOKUPVAR.finditer(text):
        consumers.append(Consumer(
            kind="ruby_var", key=match.group(1), pattern=None, file=file,
            line=_line(text, match.start()), detail="lookupvar"))
    return consumers
