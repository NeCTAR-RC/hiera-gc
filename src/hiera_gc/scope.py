...
EXCLUDED_DIRS = {"spec", "examples", "tests", "pkg", "vendor", ".git",
                 ".librarian", "fixtures"}
...
def build_scope(env: Environment, config: RunConfig) -> EnvScope:
    ...
    for directory in _modulepath(env, config, scope.warnings):
        ...
        for child in sorted(directory.iterdir()):
            if child.is_dir() and MODULE_NAME_RE.match(child.name) \
    ...
    for module_dir in scope.modules.values():
        ...
        scope.ruby_files.extend(_walk(module_dir / "lib" / "puppet",
                                      (".rb",)))
    return scope


def _modulepath(env: Environment, config: RunConfig,
                warnings: List[Warn]) -> List[Path]:
    base = [config.code_dir / "modules"]
    if VENDOR_MODULE_PATH.is_dir():
        base.append(VENDOR_MODULE_PATH)

    raw = _environment_conf_value(env.path / "environment.conf",
                                  "modulepath", warnings)
    if raw is None:
...
def _environment_conf_value(path: Path, wanted: str,
                            warnings: List[Warn]):
    if not path.is_file():
        return None
    try:
        ...
        warnings.append(Warn("parse_error", "cannot read %s: %s"
                             % (path, exc), file=str(path)))
        return None
