...
class TopKey:
    ...
    node: yaml.Node | None  # value node; None when loaded via json
    ...
    def digest(self) -> str:
        ...
        canonical = json.dumps(
            self.json_value, sort_keys=True, separators=(",", ":")
        )
...
def load_data_file(path: Path) -> LoadedDoc:
    ...
    try:
        roots = [
            r
            for r in yaml.compose_all(text, Loader=yaml.SafeLoader)
            if r is not None
        ]
        ...
        raise DataFileError(
            "YAML parse error: {}".format(str(exc).replace("\n", " ")),
    ...
    if len(roots) > 1:
        doc.problems.append(
            f"multi-document file ({len(roots)} documents); "
            "hiera reads only the first"
        )
    root = roots[0]
    ...
    for key_node, value_node in root.value:
        ...
        if not isinstance(key_node, yaml.ScalarNode):
            doc.problems.append(
                "non-scalar top-level key at line "
        ...
        if name in seen_lines:
            doc.problems.append(
                f"duplicate top-level key '{name}' "
                f"(lines {seen_lines[name]} and {line})"
            )
...
def _load_json(path: Path, text: str) -> LoadedDoc:
    ...
    try:
        ...
        raise DataFileError(f"JSON parse error: {exc}")
    ...
    for name, value in data.items():
        doc.keys.append(
            TopKey(name=str(name), line=0, node=None, json_value=value)
        )
    return doc
...
def node_digest(node: yaml.Node) -> str:
    ...
    return hashlib.sha256(
        _canonical(node, {}, set()).encode("utf-8", "replace")
    ).hexdigest()
...
def _canonical(node: yaml.Node, memo: dict, in_progress: set) -> str:
    ...
    if isinstance(node, yaml.ScalarNode):
        result = f"S({node.tag}|{len(str(node.value))}:{node.value})"
        ...
        pairs = sorted(
            f"{_canonical(k, memo, in_progress)}=>{_canonical(v, memo, in_progress)}"
            for k, v in node.value
        )
        result = "M{{{}}}".format(",".join(pairs))
    else:
        result = f"?({node!r})"
