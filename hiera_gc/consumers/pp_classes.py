...
def _parse_definition(
    tokens: list[Token], i: int, result: PPDefinitions
) -> int:
    kind = tokens[i].value
    j = i + 1
    ...
    definition = ClassDef(
        name=name_token.value, params=params, line=name_token.line, kind=kind
    )
...
def _param_from_chunk(chunk: list[Token]) -> ParamDef | None:
    depth = 0
    for token in chunk:
        if token.kind == "punct" and token.value in OPEN_BRACKETS:
            depth += 1
...
def _parse_assignment(
    ...
    value = tokens[j]
    continues = j + 1 < len(tokens) and (
        tokens[j + 1].kind == "op"
        or (tokens[j + 1].kind == "punct" and tokens[j + 1].value in "+-*/.?[")
    )
    if value.kind == "string" and not value.interpolated and not continues:
        assign = VarAssign(
            var=var.value, values=[value.value], literal=True, line=var.line
        )
        ...
        assign = VarAssign(
            var=var.value, values=[], literal=False, line=var.line
        )
    result.assignments.append(assign)
    return i + 2


def _find_selector(tokens: list[Token], j: int) -> int | None:
...
def _read_selector_value(
    tokens: list[Token], k: int, values: list[str]
) -> bool:
