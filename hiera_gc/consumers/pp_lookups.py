...
def extract_lookups(
    tokens: list[Token], file: Path, kind: str = "pp_lookup"
) -> list[Consumer]:
    ...
    while i < n:
        ...
        if (
            token.kind == "ident"
            and token.value in LOOKUP_FUNCS
            and _is_call(tokens, i)
        ):
            args = _read_args(tokens, i + 1)
            consumers.extend(
                _classify_call(token.value, args, file, token.line, kind)
            )
            i += 2
        elif (
            token.kind == "ident"
            and token.value == "Deferred"
            and _is_call(tokens, i)
        ):
            args = _read_args(tokens, i + 1)
            consumers.extend(_classify_deferred(args, file, token.line, kind))
            i += 2
        else:
            i += 1
    return consumers
...
def _classify_call(
    func: str, args: list[list[Token]], file: Path, line: int, kind: str
) -> list[Consumer]:
...
def _keys_from_arg(arg: list[Token]) -> list | None:
...
def _classify_deferred(
    args: list[list[Token]], file: Path, line: int, kind: str
) -> list[Consumer]:
    if len(args) < 2:
        return []
    head = args[0]
...
def _strip_brackets(arg: list[Token]) -> list[Token]:
    ...
    if arg and arg[0].kind == "punct" and arg[0].value == "[":
        inner = [
            t for t in arg[1:] if not (t.kind == "punct" and t.value in ("]",))
        ]
        head = []
        for token in inner:
