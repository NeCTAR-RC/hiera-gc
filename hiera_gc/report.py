...
def render_text(result: AnalysisResult, show: list[str], fixes=None) -> str:
    code_dir = getattr(result.config, "code_dir", None)
    out: list[str] = []
    out.append(
        f"hiera-gc report  (code: {code_dir}, "
    ...
    if "unused" in show:
        _render_unused(
            out, [f for f in active if f.status == "UNUSED"], code_dir
        )
    elif "stale_params" in show:
        _render_unused(
            out,
            [f for f in active if f.status == "UNUSED" and f.stale_param],
            code_dir,
            title="STALE PARAMETERS",
        )
    if "possibly_used" in show:
        _render_possibly(out, active, code_dir)
    if "redundant" in show and result.redundant:
        out.append(f"REDUNDANT OVERRIDES ({len(result.redundant)})")
        for finding in result.redundant:
            out.append(f"  {finding.key}")
            out.append(
                f"    remove:       {_location(finding.file, finding.line, code_dir)}"
            )
            out.append(
                "    identical to: {}   (envs: {})".format(
                    _location(
    ...
    if "shadowed" in show and result.shadowed:
        out.append(
            f"SHADOWED DEFINITIONS ({len(result.shadowed)})  [likely bugs]"
        )
        for finding in result.shadowed:
            out.append(f"  {finding.key}")
            out.append(
                f"    never wins:  {_location(finding.file, finding.line, code_dir)}"
            )
            out.append(
                "    shadowed by: {}   (envs: {})".format(
                    _location(
    ...
    if "orphans" in show and result.orphans:
        out.append(
            f"ORPHANED DATA FILES ({len(result.orphans)})  "
            "[unreachable via the hierarchy]"
        )
        for orphan in result.orphans:
            out.append(f"  {_rel(orphan.file, code_dir)}   ({orphan.message})")
        out.append("")
    if "stale_files" in show and result.stale_files:
        out.append(f"STALE DATA FILES ({len(result.stale_files)})")
        for stale in result.stale_files:
            out.append(f"  {_rel(stale.file, code_dir)}")
            out.append(f"    {stale.message}")
        out.append("")
    if "warnings" in show and result.warnings:
        out.append(f"WARNINGS ({len(result.warnings)})")
        for warn in result.warnings:
            location = (
                _location(warn.file, warn.line, code_dir) if warn.file else ""
            )
            out.append(
                "  [{}] {}{}".format(
                    warn.kind,
    ...
    out.append(
        f"Summary: {counts['keys']} key definitions, "
        f"{counts['unused']} unused "
        f"({counts['stale_params']} stale params), "
        f"{counts['possibly_used']} possibly used, "
        f"{counts['allowlisted']} allowlisted, "
        f"{counts['redundant']} redundant, "
        f"{counts['shadowed']} shadowed, "
        f"{counts['orphans']} orphaned files, "
        f"{counts['stale_files']} stale data files, "
...
def _render_unused(
    out: list[str], unused, code_dir, title: str = "UNUSED KEYS"
) -> None:
    if not unused:
        return
    grouped = _group_by_name(unused)
    out.append(f"{title} ({len(grouped)} keys, {len(unused)} definitions)")
    for name in sorted(grouped):
        tags = []
        for finding in grouped[name]:
            if finding.stale_param:
                tags.append(f"STALE PARAM: {finding.stale_param}")
                break
        if not tags:
            for finding in grouped[name]:
                if finding.define_shape:
                    ...
                    break
        out.append("  {}{}".format(name, f"   [{tags[0]}]" if tags else ""))
        for finding in grouped[name]:
            out.append(
                "    defined: {}   [{}]  unused in: {}".format(
...
def _render_possibly(out: list[str], findings, code_dir) -> None:
    ...
    out.append(
        f"POSSIBLY USED ({len(grouped)} keys, {len(possible)} definitions)"
    )
    for name in sorted(grouped):
        out.append(f"  {name}")
        for finding in grouped[name]:
            out.append(
                f"    defined: {_location(finding.key.file, finding.key.line, code_dir)}   [{finding.key.layer}]"
            )
            if finding.reason is not None:
                out.append(
                    "    why kept: {} {}   ({})".format(
                        finding.reason.kind,
                        finding.reason.detail,
                        _location(
                            finding.reason.file, finding.reason.line, code_dir
                        ),
                    )
                )
    out.append("")
...
def _render_fixes(out: list[str], plan, code_dir) -> None:
    ...
    out.append(
        "FIXES  (environment: {}{})".format(
            plan.env, ", dry run" if plan.dry_run else ""
        )
    )
    if not removed and not deleted:
        out.append("  nothing fixable in this environment's own data files")
    for action in removed:
        span = f"{action.start_line}"
        if action.end_line > action.start_line:
            span = f"{action.start_line}-{action.end_line}"
        out.append(
            f"  {verb_key} key '{action.key}'   {_rel(action.file, code_dir)}:{span}   [{action.finding}]"
        )
    for action in deleted:
        out.append(
            f"  {verb_file} file {_rel(action.file, code_dir)}   [{action.finding}]"
        )
    ...
    if plan.out_of_scope:
        out.append(
            "  out of scope (shared/global/module data or other "
            "environments): {}".format(
                ", ".join(
                    f"{kind}={count}"
                    for kind, count in sorted(plan.out_of_scope.items())
                )
            )
        )
    out.append("")
...
def render_json(result: AnalysisResult, show: list[str], fixes=None) -> str:
    code_dir = getattr(result.config, "code_dir", None)
    doc = {
        ...
        doc["redundant"] = [
            {
                ...
                "identical_to_line": r.anchor_line,
                "envs": r.envs,
            }
        ...
        doc["shadowed"] = [
            {
                ...
                "shadowed_by_line": s.shadow_line,
                "envs": s.envs,
            }
