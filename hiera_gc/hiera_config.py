...
def parse_hiera_config(path: Path) -> HieraConfig:
    ...
    if not isinstance(raw, dict):
        config.warnings.append(
            Warn(
                "hiera_config",
                f"{path} is not a mapping; skipped",
                file=str(path),
            )
        )
        config.usable = False
    ...
    if any(str(k).startswith(":") for k in raw):
        config.warnings.append(
            Warn(
                "hiera_config",
                f"{path} looks like hiera 3 syntax (':backends:'); only hiera 5 "
                "is supported, skipped",
                file=str(path),
            )
        )
        config.usable = False
    ...
    if version != 5:
        config.warnings.append(
            Warn(
                "hiera_config",
                f"{path} has version {version!r}, expected 5; skipped",
                file=str(path),
            )
        )
        config.usable = False
    ...
    for section, is_default_hierarchy in (
        ("hierarchy", False),
        ...
        if not isinstance(items, list):
            config.warnings.append(
                Warn(
                    "hiera_config",
                    f"{path}: {section} is not a list; skipped",
                    file=str(path),
                )
            )
            continue
...
def _backend_of(mapping: dict) -> str | None:
...
def _parse_entry(
    ...
    if backend_kind == "hocon":
        warnings.append(
            Warn(
                "unknown_backend",
                f"hierarchy entry '{name}' in {path} uses hocon_data, which hiera-gc "
        ...
        warnings.append(
            Warn(
                "unknown_backend",
                f"hierarchy entry '{name}' in {path} uses backend '{backend_name}'; lookups served "
    ...
    if not entry.patterns and entry.backend_kind in ("yaml", "json"):
        warnings.append(
            Warn(
                "hiera_config",
                f"hierarchy entry '{name}' in {path} has no path/paths/glob/globs; "
                "nothing to scan",
                file=str(path),
            )
        )
