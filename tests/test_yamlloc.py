...
def test_empty_list_root_and_multidoc_problems(tmp_path):
    ...
    assert (
        "root is not a mapping (hiera expects a hash)"
        in load_data_file(write(tmp_path, "l.yaml", "- a\n- b\n")).problems
    )
    doc = load_data_file(write(tmp_path, "m.yaml", "a: 1\n---\nb: 2\n"))
...
def test_digest_handles_recursive_aliases(tmp_path):
    doc = load_data_file(
        write(tmp_path, "a.yaml", "selfish: &s\n  inner: *s\n")
    )
...
def test_iter_string_scalars(tmp_path):
    ...
    strings = {
        value for value, _ in iter_string_scalars(keys_of(doc)["k"].node)
    }
