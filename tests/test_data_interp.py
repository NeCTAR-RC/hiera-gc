...
def test_lookup_alias_hiera_interpolations(tmp_path):
    consumers, _, warnings = extract(tmp_path, """\
a: "%{lookup('other_key')}"
b: "%{alias('_secret_key')}"
c: "%{hiera('legacy_key')}"
d: "%{literal('%')}{escaped}"
...
def test_variable_interpolations(tmp_path):
    consumers, _, _ = extract(tmp_path, """\
a: "%{::profile::base::dns}"
...
""")
    ...
    assert keys == {"profile::base::dns",
                    "nectar::profile::common::region"}
...
def test_lookup_options_entries(tmp_path):
    _, entries, warnings = extract(tmp_path, """\
lookup_options:
  exact::key:
    merge: deep
  "^profile::.*::extras$":
...
""")
    ...
    assert set(by_name) == {"exact::key", "profile::.*::extras$",
                            "no_merge::key"}
...
def test_lookup_options_not_consuming_other_keys(tmp_path):
    consumers, entries, _ = extract(tmp_path, """\
lookup_options:
  some::key:
    merge: unique
""")
